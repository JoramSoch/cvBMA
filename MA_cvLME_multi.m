function MA_cvLME_multi(SPM, data, mode, AnC) % Release cvBMA [1]
% _
...
for g = 1:s
    ...
    %---------------------------------------------------------------------%
    log_py(m_ind) = ME_GLM_NG_LME(S(g).P, S(g).L0, S(g).a0, S(g).b0, S(g).Ln, S(g).an, S(g).bn);
    if AnC
        [Acc_ym(m_ind), Com_ym(m_ind)] = ME_GLM_NG_AnC(S(g).X, S(g).P, S(g).m0, S(g).L0, S(g).a0, S(g).b0, S(g).mn, S(g).Ln, S(g).an, S(g).bn, sprintf('Compute accuracy and complexity for session %d',g));
    end;
...
%-------------------------------------------------------------------------%
H.fname   = strcat('MA_cvLME_',data,'_',mode,'.nii');
H.descrip = sprintf('MA_cvLME_multi: cross-validated log model evidence for general linear model with normal-gamma priors (GLM-NG)');
spm_write_vol(H,reshape(cvLME,m_dim));
for g = 1:s
    H.fname   = strcat('MA_cvLME_',data,'_',mode,'_S',int2str(g),'.nii');
    H.descrip = sprintf('MA_cvLME_multi: log model evidence for session %s based on priors estimated from other sessions (data: %s; mode: %s)',int2str(g),data,mode);
...
%-------------------------------------------------------------------------%
if AnC
    H.fname   = strcat('MA_cvAcc_',data,'_',mode,'.nii');
    H.descrip = sprintf('MA_cvLME_multi: cross-validated model accuracy for general linear model with normal-gamma priors (GLM-NG)');
    spm_write_vol(H,reshape(cvAcc,m_dim));
    H.fname   = strcat('MA_cvCom_',data,'_',mode,'.nii');
    H.descrip = sprintf('MA_cvLME_multi: cross-validated model complexity for general linear model with normal-gamma priors (GLM-NG)');
    spm_write_vol(H,reshape(cvCom,m_dim));
    for g = 1:s
        H.fname   = strcat('MA_cvAcc_',data,'_',mode,'_S',int2str(g),'.nii');
        H.descrip = sprintf('MA_cvLME_multi: model accuracy for session %s based on priors estimated from other sessions (data: %s; mode: %s)',int2str(g),data,mode);
        spm_write_vol(H,reshape(S(g).Acc,m_dim));
        H.fname   = strcat('MA_cvCom_',data,'_',mode,'_S',int2str(g),'.nii');
        H.descrip = sprintf('MA_cvLME_multi: model complexity for session %s based on priors estimated from other sessions (data: %s; mode: %s)',int2str(g),data,mode);
